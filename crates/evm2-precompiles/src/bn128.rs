use crate::{B160, Error, Precompile, PrecompileAddress, PrecompileResult, primitives::U256};
use alloc::vec::Vec;
...
pub mod pair {
    ...
    pub const ISTANBUL: PrecompileAddress = PrecompileAddress(
        ADDRESS,
        Precompile::Standard(|input: &[u8], target_gas: u64| -> PrecompileResult {
            super::run_pair(input, ISTANBUL_PAIR_PER_POINT, ISTANBUL_PAIR_BASE, target_gas)
        }),
    );
    ...
    pub const BYZANTIUM: PrecompileAddress = PrecompileAddress(
        ADDRESS,
        Precompile::Standard(|input: &[u8], target_gas: u64| -> PrecompileResult {
            super::run_pair(input, BYZANTIUM_PAIR_PER_POINT, BYZANTIUM_PAIR_BASE, target_gas)
        }),
    );
}
...
fn run_add(input: &[u8]) -> Result<Vec<u8>, Error> {
    ...
    if let Some(sum) = AffineG1::from_jacobian(p1 + p2) {
        sum.x().into_u256().to_big_endian(&mut output[..32]).unwrap();
        sum.y().into_u256().to_big_endian(&mut output[32..]).unwrap();
    }
...
fn run_pair(
    ...
    let output = if input.is_empty() {
        ...
        let mul = vals.into_iter().fold(Gt::one(), |s, (a, b)| s * bn::pairing(a, b));

        if mul == Gt::one() { U256::from(1) } else { U256::ZERO }
    };
