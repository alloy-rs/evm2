...
use evm2_precompiles::{eip4844::VERSIONED_HASH_VERSION_KZG, hex, kzg_point_evaluation::run};
...
pub fn add_benches<M: Measurement>(group: &mut BenchmarkGroup<'_, M>) {
    ...
    group.bench_function("kzg precompile", |b| b.iter(|| run(&kzg_input, gas).unwrap()));
}
