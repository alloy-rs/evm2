...
- `structured_compare`: arbitrary serialized `EvmCase` inputs.
- `generated_compare`: deterministic structured generator inputs.
- `validation_compare`: transaction-validation boundary cases with equivalent rejection handling.
- `evm_smith_compare` and `evm_smith_arbitrary_compare`: evm-smith generated bytecode.
- `precompile_compare`: precompile-focused cases.
- `lifecycle_compare`: account lifecycle-focused cases.
- `jit_vs_interpreter`: evm2 interpreter vs evm2 JIT bytecode execution, enabled with the `jit` feature.
...
Run the JIT comparison target:

```sh
cargo +nightly fuzz run --features jit jit_vs_interpreter
```
