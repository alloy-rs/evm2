...
impl AccountBal {
    ...
    pub fn update(&mut self, bal_index: BlockAccessIndex, account: &AccountChange) {
        ...
        if account.is_selfdestructed() {
            ...
            self.account_info.update(bal_index, &original, &empty_info);
            // Selfdestruct wipes all storage to zero, record writes accordingly.
            self.storage.update_selfdestruct(bal_index, &account.storage);
            return;
        }
...
impl StorageBal {
    ...
    #[inline]
    pub fn update_selfdestruct(
        &mut self,
        bal_index: BlockAccessIndex,
        storage: &U256Map<Tracked<Word>>,
    ) {
        for (key, value) in storage {
            self.storage.entry(*key).or_default().update(bal_index, &value.original, U256::ZERO);
        }
    }
