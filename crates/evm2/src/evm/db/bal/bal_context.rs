...
/// [`CacheDB`](crate::evm::CacheDB)) is not itself BAL-oriented. It mirrors revm's `BalState`, which
/// likewise lives on the database wrapper rather than the journaled state.
///
...
impl BalContext {
    ...
    /// `changes` are recorded as BAL reads; changed ones as writes. Mirrors revm's
    /// `BalState::commit`.
    #[inline]
    ...
    /// `account`, mirroring revm's `BalState::basic`.
    ///
    ...
    /// Resolves storage slot `key` for `address` from the attached read BAL at the current index,
    /// mirroring revm's `BalState::storage`.
    ///
