...
use alloc::vec::Vec;
use alloy_primitives::map::{AddressMap, AddressSet, B256Map, U256Map};
...
pub struct StateChanges {
    ...
    /// Addresses of accounts created during the transaction.
    ///
    /// This includes accounts created at an address that already held a balance (a created account
    /// whose [`Tracked::original`] in [`Self::accounts`] is therefore `Some`), which cannot be
    /// distinguished from a plain modification by the account delta alone.
    pub created: AddressSet,
