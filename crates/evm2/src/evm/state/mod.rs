...
mod tests {
    use super::*;
    use crate::{SpecId, Version, evm::InMemoryDB};
    use core::convert::Infallible;

    #[test]
    fn pre_eip161_materialized_empty_account_is_created() {
        let address = Address::with_last_byte(1);
        let mut state = State::new(InMemoryDB::default());
        state.account(&address, false).unwrap().touch();
        let version = Version::base(SpecId::FRONTIER);
        ...
        impl StateChangeSink for Sink {
            type Error = Infallible;

            fn account(&mut self, change: AccountChangeRef<'_>) -> Result<(), Self::Error> {
                if change.address == self.address {
                    self.created = Some(change.created);
                }
                Ok(())
            }
        }
        ...
        let mut sink = Sink { address, created: None };
        pending.visit(&mut sink).unwrap();
        assert_eq!(sink.created, Some(true));
    }
}
