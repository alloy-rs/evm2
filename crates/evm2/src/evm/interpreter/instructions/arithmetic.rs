...
#[instruction]
pub(in crate::evm::interpreter) fn add([a, b]: [Word]) -> out {
    *out = a.wrapping_add(b);
}

#[instruction]
pub(in crate::evm::interpreter) fn mul([a, b]: [Word]) -> out {
    *out = a.wrapping_mul(b);
}

#[instruction]
pub(in crate::evm::interpreter) fn sub([a, b]: [Word]) -> out {
    *out = a.wrapping_sub(b);
}

#[instruction]
pub(in crate::evm::interpreter) fn div([a, b]: [Word]) -> out {
    *out = if b.is_zero() { Word::ZERO } else { a.wrapping_div(b) };
}

#[instruction]
pub(in crate::evm::interpreter) fn sdiv([a, b]: [Word]) -> out {
    *out = i256_div(a, b);
}

#[instruction]
pub(in crate::evm::interpreter) fn rem([a, b]: [Word]) -> out {
    *out = if b.is_zero() { Word::ZERO } else { a.wrapping_rem(b) };
}

#[instruction]
pub(in crate::evm::interpreter) fn smod([a, b]: [Word]) -> out {
    *out = i256_mod(a, b);
}

#[instruction]
pub(in crate::evm::interpreter) fn addmod([a, b, n]: [Word]) -> out {
    *out = a.add_mod(b, n);
}

#[instruction]
pub(in crate::evm::interpreter) fn mulmod([a, b, n]: [Word]) -> out {
    *out = a.mul_mod(b, n);
}

#[instruction]
pub(in crate::evm::interpreter) fn exp([a, b]: [Word]) -> out {
    *out = a.wrapping_pow(b);
}

#[instruction]
pub(in crate::evm::interpreter) fn signextend([ext, value]: [Word]) -> out {
    if ext < 31 {
