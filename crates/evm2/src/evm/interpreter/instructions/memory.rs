...
#[instruction]
pub(in crate::evm::interpreter) fn mload(cx: _, [offset]: [Word]) -> Result<out> {
...
#[instruction]
pub(in crate::evm::interpreter) fn mstore(cx: _, [offset, value]: [Word]) -> Result {
    let offset = as_usize(offset)?;
    resize_memory(cx.gas, cx.state.memory(), offset, 32)?;
...
#[instruction]
pub(in crate::evm::interpreter) fn mstore8(cx: _, [offset, value]: [Word]) -> Result {
    let offset = as_usize(offset)?;
...
#[instruction]
pub(in crate::evm::interpreter) fn msize(cx: _) -> out {
    *out = Word::from(cx.state.memory().len());
}

#[instruction]
pub(in crate::evm::interpreter) fn mcopy(cx: _, [dst, src, len]: [Word]) -> Result {
