...
use crate::{
    EvmTypes,
    env::TxEnvExt,
    ethereum::{InitialFrame, prepare_initial_frame, runtime_oog_result, settle_initial_frame_gas},
    ...
    pub fn execute_system_call(&mut self, tx: SystemTx) -> HandlerResult<TxResult<T>> {
        ...
        let result = match prepare_initial_frame(
            ...
            Some(InitialFrame { mut message, charged_state_gas }) => {
                let mut result = Host::execute_message(self, &tx_env, &mut message);
                settle_initial_frame_gas(&mut tx_gas, &mut result, charged_state_gas);
                result
            }
            None => runtime_oog_result(SYSTEM_CALL_GAS_LIMIT, reservoir),
        };
