...
use super::{END_OFFSET, Entry, Kernel, PUSHX, analyze_blocks, lanes, uncarried};
...
const JUMPDEST: i8 = op::JUMPDEST as i8;
const BIAS: i8 = 0x70;
...
fn next_entry16(exits: __m128i, entry: __m128i) -> __m128i {
    ...
    _mm_max_epu8(_mm_shuffle_epi8(_mm_sub_epi8(exits, block), entry), _mm_sub_epi8(entry, block))
}
...
impl Kernel for Sse41x4 {
    ...
    unsafe fn block(ptr: *const u8, entry: &mut __m128i) -> u64 {
        ...
        let starts = (0..4).fold(0, |acc, i| acc | (starts16(ops[i], entry) as u64) << (i * 16));
        starts & jumpdests
    }
}
...
impl Kernel for Avx2 {
    ...
    unsafe fn block(ptr: *const u8, entry: &mut __m128i) -> u32 {
        ...
        let n0 = _mm256_add_epi8(_mm256_max_epi8(ops, _mm256_set1_epi8(PUSHX)), END_OFFSETS16X2);
...
impl Kernel for Avx512Vbmix2 {
    ...
    unsafe fn block(ptr: *const u8, entry: &mut __m512i) -> u128 {
        ...
        let (ops_a, ops_b) =
            unsafe { (_mm512_loadu_si512(ptr.cast()), _mm512_loadu_si512(ptr.add(64).cast())) };
...
mod tests {
    use super::*;
    use crate::bytecode::analysis::tests::check_simd;
