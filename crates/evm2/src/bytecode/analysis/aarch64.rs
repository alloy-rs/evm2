...
impl Kernel for Neon {
    ...
    unsafe fn block(ptr: *const u8, entry: &mut uint8x16_t) -> u16 {
        ...
        let starts = vandq_u8(starts16(&n, *entry), vceqq_u8(ops, vdupq_n_u8(op::JUMPDEST)));
...
impl Kernel for Neonx4 {
    ...
    unsafe fn block(ptr: *const u8, entry: &mut uint8x16_t) -> u64 {
        ...
        let jumpdests = ops.map(|ops| vceqq_u8(ops, vdupq_n_u8(op::JUMPDEST)));
        let any_jumpdest =
            vorrq_u8(vorrq_u8(jumpdests[0], jumpdests[1]), vorrq_u8(jumpdests[2], jumpdests[3]));
