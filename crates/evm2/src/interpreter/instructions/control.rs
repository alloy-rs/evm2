...
mod tests {
    ...
    #[test]
    fn stop_opcode() {
        let interpreter = run(RunConfig::new([opcode::STOP]));
        assert!(matches!(interpreter.err, InstrStop::Stop));

        let interpreter = run(RunConfig::new([opcode::STOP, opcode::INVALID]));
        assert!(matches!(interpreter.err, InstrStop::Stop));
    }

    #[test]
    fn invalid_opcode() {
        let interpreter = run(RunConfig::new([opcode::INVALID]));
    ...
    #[test]
    fn pc_opcode() {
        let interpreter = run(RunConfig::new([opcode::PC, opcode::JUMPDEST, opcode::STOP]));
        assert!(matches!(interpreter.err, InstrStop::Stop));
        assert_eq!(interpreter.stack(), [0]);

        let interpreter = run(RunConfig::new([opcode::JUMPDEST, opcode::PC, opcode::STOP]));
        assert!(matches!(interpreter.err, InstrStop::Stop));
    ...
    #[test]
    fn gas_opcode() {
        let interpreter = run(RunConfig::new([opcode::GAS, opcode::STOP]));
        assert!(matches!(interpreter.err, InstrStop::Stop));
        assert_eq!(interpreter.stack().len(), 1);
        assert!(interpreter.stack()[0] < Word::from(10_000));

        let interpreter = run(RunConfig::new([opcode::GAS, opcode::GAS, opcode::STOP]));
        assert!(matches!(interpreter.err, InstrStop::Stop));
    ...
    #[test]
    fn jumpdest_opcode() {
        let interpreter = run(RunConfig::new([opcode::JUMPDEST, opcode::STOP]));
        assert!(matches!(interpreter.err, InstrStop::Stop));
        assert!(interpreter.stack().is_empty());

        let interpreter = run(RunConfig::new([opcode::JUMPDEST, opcode::JUMPDEST, opcode::STOP]));
        assert!(matches!(interpreter.err, InstrStop::Stop));
    }

    #[test]
    fn return_opcode() {
        let mut interpreter = run_stack([0, 0], opcode::RETURN);
        assert!(matches!(interpreter.err, InstrStop::Return));
        assert!(interpreter.memory(0, 0).is_empty());
        assert!(interpreter.output().is_empty());

        let mut interpreter = run_stack([0, 1], opcode::RETURN);
        assert!(matches!(interpreter.err, InstrStop::Return));
        ...
        let interpreter = run_stack([Word::from(0), Word::MAX], opcode::RETURN);
        assert!(matches!(interpreter.err, InstrStop::InvalidOperandOOG));
    }

    #[test]
    fn revert_opcode() {
        let mut interpreter = run_stack([0, 0], opcode::REVERT);
        assert!(matches!(interpreter.err, InstrStop::Revert));
        assert!(interpreter.memory(0, 0).is_empty());
        assert!(interpreter.output().is_empty());

        let mut interpreter = run_stack([2, 3], opcode::REVERT);
        assert!(matches!(interpreter.err, InstrStop::Revert));
        ...
        push(&mut code, Word::from(2));
        push(&mut code, Word::from(4));
        code.push(opcode::REVERT);
        let interpreter = run(RunConfig::new(code));
        assert!(matches!(interpreter.err, InstrStop::Revert));
        assert_eq!(interpreter.output(), [0xcd, 0]);

        let interpreter = run_stack([Word::from(0), Word::MAX], opcode::REVERT);
        assert!(matches!(interpreter.err, InstrStop::InvalidOperandOOG));
    }
}
