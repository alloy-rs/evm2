...
#[cfg(test)]
std::thread_local! {
    static AUTHORITY_RECOVERY_ATTEMPTS: Cell<usize> = const { Cell::new(0) };
}

/// Resets the test-only signed authorization recovery-attempt counter.
#[cfg(test)]
pub(super) fn reset_authority_recovery_attempts() {
    AUTHORITY_RECOVERY_ATTEMPTS.set(0);
}

/// Returns the test-only signed authorization recovery-attempt counter.
#[cfg(test)]
pub(super) fn authority_recovery_attempts() -> usize {
    AUTHORITY_RECOVERY_ATTEMPTS.get()
}

#[cfg(test)]
fn note_authority_recovery_attempt() {
    AUTHORITY_RECOVERY_ATTEMPTS.set(AUTHORITY_RECOVERY_ATTEMPTS.get() + 1);
}
    ...
    pub fn authority(&self) -> Option<Address> {
        match self {
            Self::Signed(authorization) => {
                #[cfg(test)]
                note_authority_recovery_attempt();

                authorization.recover_authority().ok()
            }
