use super::{
    GasSettlement, access_list_counts, charge_upfront, effective_gas_price, floor_gas,
    initial_message, intrinsic_gas, rollback_failed_execution, settle_gas_with_extra_refund,
    validate_block_gas_limit, validate_floor_gas, validate_gas_price, validate_intrinsic_gas,
...
pub(super) fn handle<T: EvmTypes<Host = Evm<T>>>(
    ...
    Ok(settle_gas_with_extra_refund(
        req.host,
        GasSettlement {
            spec_id,
            caller,
...
fn apply_auth_list<T: EvmTypes<Host = Evm<T>>>(
    ...
    for authorization in authorizations {
        ...
        let existed = host.state.account_info(authority).is_some();
        let authority_info = host.state.account_info(authority).unwrap_or_default();
