...
pub(crate) const G1_ADD_ADDRESS: Address = u64_to_address(0x0b);
/// G1 msm precompile address
pub(crate) const G1_MSM_ADDRESS: Address = u64_to_address(0x0c);
/// G2 add precompile address
pub(crate) const G2_ADD_ADDRESS: Address = u64_to_address(0x0d);
/// G2 msm precompile address
pub(crate) const G2_MSM_ADDRESS: Address = u64_to_address(0x0e);
/// Pairing precompile address
pub(crate) const PAIRING_ADDRESS: Address = u64_to_address(0x0f);
/// Map fp to g1 precompile address
pub(crate) const MAP_FP_TO_G1_ADDRESS: Address = u64_to_address(0x10);
/// Map fp2 to g2 precompile address
pub(crate) const MAP_FP2_TO_G2_ADDRESS: Address = u64_to_address(0x11);

/// G1_ADD_BASE_GAS_FEE specifies the amount of gas needed
/// to perform the G1_ADD precompile.
pub(crate) const G1_ADD_BASE_GAS_FEE: u64 = 375;
...
pub(crate) const G1_MSM_BASE_GAS_FEE: u64 = 12000;
...
/// this precompile with a single point and a scalar.
pub(crate) const G2_MSM_BASE_GAS_FEE: u64 = 22500;
...
pub(crate) const PAIRING_OFFSET_BASE: u64 = 37700;
...
pub(crate) const PAIRING_MULTIPLIER_BASE: u64 = 32600;

/// Discounts table for G1 MSM as a vector of pairs `[k, discount]`.
pub(crate) static DISCOUNT_TABLE_G1_MSM: [u16; 128] = [
...
pub(crate) static DISCOUNT_TABLE_G2_MSM: [u16; 128] = [
...
pub(crate) const G1_LENGTH: usize = 2 * FP_LENGTH;
...
pub(crate) const PADDED_G1_LENGTH: usize = 2 * PADDED_FP_LENGTH;
...
pub(crate) const FP2_LENGTH: usize = 2 * FP_LENGTH;
...
/// Note: This is the quadratic extension of Fp, and by definition
/// means we need 2 Fp elements.
pub(crate) const PADDED_FP2_LENGTH: usize = 2 * PADDED_FP_LENGTH;
...
/// This is an element in the scalar field of BLS12-381.
pub(crate) const SCALAR_LENGTH_BITS: usize = SCALAR_LENGTH * 8;
...
pub(crate) const G1_ADD_INPUT_LENGTH: usize = 2 * PADDED_G1_LENGTH;
...
/// of these pairs.
pub(crate) const G1_MSM_INPUT_LENGTH: usize = PADDED_G1_LENGTH + SCALAR_LENGTH;

/// G2_LENGTH specifies the number of bytes needed to represent a G2 element.
///
/// Note: A G2 element contains 2 Fp^2 elements.
pub(crate) const G2_LENGTH: usize = 2 * FP2_LENGTH;
...
pub(crate) const PADDED_G2_LENGTH: usize = 2 * PADDED_FP2_LENGTH;
...
pub(crate) const G2_ADD_INPUT_LENGTH: usize = 2 * PADDED_G2_LENGTH;
...
/// of these pairs.
pub(crate) const G2_MSM_INPUT_LENGTH: usize = PADDED_G2_LENGTH + SCALAR_LENGTH;

/// PAIRING_INPUT_LENGTH specifies the number of bytes that each Pairing input pair should have.
///
/// Note: An Pairing input-pair is a G2 element and a G1 element. The input to the Pairing will have
/// `n` of these pairs.
pub(crate) const PAIRING_INPUT_LENGTH: usize = PADDED_G1_LENGTH + PADDED_G2_LENGTH;

/// FP_PAD_BY specifies the number of bytes that an FP_ELEMENT is padded by to make it 32 byte
/// aligned.
///
/// Note: This should be equal to PADDED_FP_LENGTH - FP_LENGTH.
pub(crate) const FP_PAD_BY: usize = 16;
...
pub(crate) const TRUSTED_SETUP_TAU_G2_BYTES: [u8; 96] = hex!(
...
fn check_discount_table_invariant_holds() {
    ...
    //
    // First it explicitly says that G1 Multiplication costs 12000 Gas and G2 Multiplication costs
    // 22500.
    //
    ...
    // Note: We could also more robustly check this by defining the G1/G2 Scalar multiplication
    // constants from the EIP and checking that they equal to the value computed by
    // `msm_required_gas` when k==1
}
