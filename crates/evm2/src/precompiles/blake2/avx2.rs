...
// - Changed compress_block signature for EIP-152: takes (rounds, words, m, t, f) instead of (block,
//   words, count, last_block, last_node), with variable round count.
...
// - Due to an LLVM bug (https://bugs.llvm.org/show_bug.cgi?id=44379), this version performs better
//   on recent x86 chips.
// - LLVM is able to optimize this version to AVX-512 rotation instructions when those are enabled.
...
pub(super) unsafe fn compress(
    ...
    unsafe {
        ...
        let block: &[u8; BLOCKBYTES] = core::mem::transmute(m);
