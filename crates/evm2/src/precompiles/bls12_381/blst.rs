...
///
/// - If the x or y coordinate do not represent a canonical field element, an error is returned. See
///   [read_fp] for more information.
...
pub(crate) fn p1_add_affine_bytes(
    a: G1Point,
    b: G1Point,
) -> Result<[u8; G1_LENGTH], crate::precompiles::PrecompileHalt> {
    let (a_x, a_y) = a;
    let (b_x, b_y) = b;
    // Parse first point
...
pub(crate) fn p2_add_affine_bytes(
    a: G2Point,
    b: G2Point,
) -> Result<[u8; G2_LENGTH], crate::precompiles::PrecompileHalt> {
...
pub(crate) fn p1_msm_bytes(
    point_scalar_pairs: impl Iterator<Item = Result<G1PointScalar, crate::precompiles::PrecompileHalt>>,
) -> Result<[u8; G1_LENGTH], crate::precompiles::PrecompileHalt> {
...
pub(crate) fn p2_msm_bytes(
    point_scalar_pairs: impl Iterator<Item = Result<G2PointScalar, crate::precompiles::PrecompileHalt>>,
) -> Result<[u8; G2_LENGTH], crate::precompiles::PrecompileHalt> {
    let (lower, _) = point_scalar_pairs.size_hint();
...
/// pairing_check_bytes performs a pairing check on a list of G1 and G2 point pairs taking byte
/// inputs.
#[inline]
pub(crate) fn pairing_check_bytes(
    pairs: &[PairingPair],
) -> Result<bool, crate::precompiles::PrecompileHalt> {
