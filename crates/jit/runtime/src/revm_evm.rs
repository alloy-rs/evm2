...
use crate::runtime::{
    JitBackend, LookupDecision, LookupRequest, RuntimeCacheKey, spec::from_revm_spec_id,
};
    ...
    fn frame_run(
        ...
        let decision = self.lookup_cache.entry(code_hash).or_insert_with(|| {
            ...
            self.backend.lookup(LookupRequest {
                key: RuntimeCacheKey { code_hash, spec_id: from_revm_spec_id(spec_id) },
                code,
            })
        });
...
mod tests {
    ...
    fn jit_evm_non_blocking() {
        ...
        loop {
            if backend.get_compiled(code_hash, from_revm_spec_id(SpecId::CANCUN)).is_some() {
                break;
            }
