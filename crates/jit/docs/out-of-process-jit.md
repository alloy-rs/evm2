...
- `RuntimeConfig::default()` uses plain in-process defaults. `JitBackend::new` applies `RuntimeConfig::with_env_overrides()`, which recognizes `EVM2_JIT_MODE=out-of-process` and `EVM2_JIT_MODE=in-process`; other spellings are rejected. `EVM2_JIT_HELPER_PATH` overrides the helper executable path. Test harnesses should point this at a binary that calls `evm2-jit::runtime::maybe_run_jit_helper()` at startup, such as `target/debug/evm2` built with the `jit` feature.
- `EVM2_JIT_HELPER_MEMORY_LIMIT_BYTES` applies a Unix `RLIMIT_AS` limit to helper processes before `exec`.
- `EVM2_JIT_HELPER_CPU_COUNT` limits helper CPU affinity on Linux before `exec`.
