use evm2_jit_statetest::compiled::CompileMode;
...
fn collect_tests(path_fn: fn() -> Option<PathBuf>, label: &str) -> Vec<PathBuf> {
    ...
    evm2_jit_statetest::find_all_json_tests(&path)
}
...
fn run_state_tests(mode: CompileMode) {
    ...
    if let Some(mut path) = evm2_jit_statetest::get_general_state_tests_path() {
        ...
        test_files.extend(evm2_jit_statetest::find_all_json_tests(&path));
    } else {
        eprintln!(
    ...
    if is_ci() && !matches!(mode, CompileMode::Aot) {
        test_files.extend(collect_tests(
            evm2_jit_statetest::get_exec_spec_stable_state_tests_path,
            "exec-spec-tests stable",
        ));
        test_files.extend(collect_tests(
            evm2_jit_statetest::get_exec_spec_develop_state_tests_path,
            "exec-spec-tests develop",
        ));
        test_files.extend(collect_tests(
            evm2_jit_statetest::get_legacy_cancun_state_tests_path,
            "legacy Cancun",
        ));
        test_files.extend(collect_tests(
            evm2_jit_statetest::get_legacy_constantinople_state_tests_path,
    ...
    evm2_jit_statetest::compiled::run(test_files, single_thread, false, mode).unwrap();
}
...
fn run_blockchain_tests() {
    ...
    test_files.extend(collect_tests(
        evm2_jit_statetest::get_exec_spec_stable_blockchain_tests_path,
        "exec-spec-tests stable blockchain_tests",
    ));
    test_files.extend(collect_tests(
        evm2_jit_statetest::get_exec_spec_develop_blockchain_tests_path,
    ...
    evm2_jit_statetest::btest::run_btests(test_files).unwrap();
}
