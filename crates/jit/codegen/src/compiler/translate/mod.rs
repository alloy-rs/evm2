...
impl<'a, B: Backend> FunctionCx<'a, B> {
    ...
    pub(super) fn translate(
        ...
        // Initialize stack length in the entry path.
    ...
    fn translate_inst(&mut self, inst: Inst) -> Result<()> {
            ...
            && let Some(const_out) = self.bytecode.const_output(inst)
        {
            ...
            debug_assert!(!data.is_recursive_frame_opcode() && !data.is_branching());
    ...
    /// materialization (e.g. `materialize_live_stack` before a branch) can flush
...
impl<B: Backend> FunctionCx<'_, B> {
    ...
    fn call_ir_builtin(
        ...
        let linkage = evm2_jit_backend::Linkage::Private;
