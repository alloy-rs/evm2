...
fn run_call_then_push<B: Backend>(compiler: &mut EvmCompiler<B>) {
    ...
    let f = unsafe { compiler.jit("resume_call", bytecode, from_revm_spec_id(DEF_SPEC)) }.unwrap();
...
fn run_call_then_return<B: Backend>(compiler: &mut EvmCompiler<B>) {
    ...
    compiler.inspect_stack(true);
    let f =
        unsafe { compiler.jit("resume_return", bytecode, from_revm_spec_id(DEF_SPEC)) }.unwrap();
...
fn run_call_returndatasize<B: Backend>(compiler: &mut EvmCompiler<B>) {
    ...
    unsafe { compiler.clear() }.unwrap();
    compiler.inspect_stack(true);
    let f = unsafe { compiler.jit("resume_rds", bytecode, from_revm_spec_id(DEF_SPEC)) }.unwrap();
...
fn run_call_pop_push_sload_stack_len<B: Backend>(compiler: &mut EvmCompiler<B>) {
    ...
    compiler.inspect_stack(true);
    let f =
        unsafe { compiler.jit("pop_push_sload", bytecode, from_revm_spec_id(DEF_SPEC)) }.unwrap();
